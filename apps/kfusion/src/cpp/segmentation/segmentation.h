...
struct instance_seg{
  ...
  instance_seg(const int class_id, const cv::Mat& instance_mask, const All_Prob_Vect& all_prob)
      : class_id_(class_id),
  ...
  /// @param generalized output generalized label map
  ...
  /// \brief Merge two instances (same semantic class) together
...
//  int find_classID_from_instanceID(const int& instance_id, bool info) const;
...
//  static constexpr float geometric_lambda = 0.20f;
  ...
  static constexpr float geom_threshold = 0.001f;   // smoothenss(大), edgeness(小) // 0.001f  // 0.0005f;
  static constexpr float geometric_lambda = 0.05f;  //0.05f //0.001f

  // Let's oversegment everything
  static constexpr uint geomtric_component_size = 0;  // ori=0
  const std::set<int> filter_class_id_ = {60 /*bed*/,61 /*dinning table*/, 62 /*toilet*/, 73 /*refrigerator*/};
 ...
 private:
  ...
  // The format of class id would be 1D integer, with the element of the class id
  std::vector<int> load_class_ids(std::string class_path);

  // The format of class probability would be #Object * #Class float
  std::vector<All_Prob_Vect> load_class_prob(const std::string prob_path, uint &class_num);

  // Load mask from all frame information
 ...
 public:
  ...
  static constexpr float new_model_threshold = 0.01f;  // 0.01f or 0.2f
  bool refine_human_boundary_;
  bool semanticfusion = true;
  uint geom_dilute_size = 7;//9  7
