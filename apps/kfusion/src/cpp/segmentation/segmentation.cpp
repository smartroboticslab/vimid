...
void instance_seg::generalize_label(cv::Mat& gener_inst_mask) const{
  ...
  // cv::imshow("instance_mask", instance_mask_);
  cv::dilate(instance_mask_, mask_with_bg, element);
  // cv::imshow("mask_with_bg", mask_with_bg);
  ...
  for (auto instance_it = pair_instance_seg_.begin(); instance_it !=
    ...
    if ( class_id > 0) {
      ...
      name << "/home/ryf/slam/vimid/apps/kfusion/cmake-build-debug/debug/"
           <<str<<"/frame_"<< frame << "_object_id_" <<index
           <<"_class_id_" <<class_id<<".png";
      cv::imwrite(name.str(), labelMask);
    } else {
      std::ostringstream name;
      name << "/home/ryf/slam/vimid/apps/kfusion/cmake-build-debug/debug/"
           <<str<<"/frame_"<< frame << "_object_id_" <<index<<".png";
      cv::imwrite(name.str(), labelMask);
    }
  }
}
...
SegmentationResult Segmentation::compute_geom_edges(const float * depth, const uint2 inSize, const float4 k){
  ...
  cv::Mat out;
  noedge_map.copyTo(out);
  cv::threshold(out, out, 0.5, 255, cv::THRESH_BINARY);
  // cv::imshow("noEdgeMap", out);
...
//  cv::drawContours(edge_filled, edge_contours, -1, cv::Scalar(255), CV_FILLED);
  ...
  int nLabels = cv::connectedComponentsWithStats(noedge_map, geomtric_label, label_stats, label_centroids,
                                                 connectivity);
  // std::cout << geomtric_label << std::endl;
  // geomtric_label.copyTo(out);
  // cv::imshow("geonetric_label", out);
  // cv::waitKey(0);
...
//      geometric_segmentation.class_mask.push_back(maskImage);
  ...
  for (int label_id = 0; label_id < nLabels; ++label_id) {
    if (label_stats.at<int>(label_id, cv::CC_STAT_AREA)>geomtric_component_size){
      ...
      cv::threshold(maskImage, out, 0.5, 255, cv::THRESH_BINARY);
      // cv::imshow("Mask", out);
      // cv::waitKey(0);

      // dirty: dilate mask a little bit
      cv::Mat element = cv::getStructuringElement(cv::MORPH_ELLIPSE,cv::Size(geom_dilute_size, geom_dilute_size));
      cv::dilate(maskImage, maskImage, element);
      // std::cout << "geo_mask id: " << label_id <<" size: " << maskImage.size() << std::endl;
...
void Segmentation::mergeLabels(SegmentationResult& mergedSeg,
  ...
  for (auto src_mask = srcSeg.pair_instance_seg_.begin();
    ...
    if (max_overlap > threshold){
      // cv::imshow("geometric", src_mask_mat);
      // cv::waitKey(0);
      ...
      if ( find_merged_id == mergedSeg.pair_instance_seg_.end()) {
        ...
        // the class is already a mask component, merged them together
        // update the mask
...
//        mergedSeg.instance_id_all_prob_.insert(*merged_instance_class_prob);
        ...
        // cv::morphologyEx(merged_id_find->second.instance_mask_, merged_id_find->second.instance_mask_, cv::MORPH_OPEN, element);
      }
    }
  }
...
//                                 std::map<int, cv::Mat> &inclusion_map,
//                                 uint frame){
//   bool hasnewmodel = false;
//
//   // In case that model is whole background
...
//     // newModel.reset();
//
//     // now remove human from mask list (background already removed)
...
//
//     std::cout << "Now come to: " << mask_class_id << "!!!!!!" << std::endl;
//
...
//     bool if_exist = false;
//     bool is_match = false;    // check if the mask matches one of the model
//     int max_iou = 0;  // used for save the maximum IoU
//     std::set<int> inclusion_set;
//
...
//       if (model_class_id == mask_class_id) if_exist = true;   // if none of the model associates with mask, directly generate new model
//       // std::cout<<"src_mask_mat: "<<src_mask_mat.size()<<std::endl;
//       // std::cout<<"dst_mask_mat: "<<dst_mask_mat.size()<<std::endl;
//
//       cv::Mat overlap_mask;
...
//       cv::bitwise_or(mask_instance_mask, model_instance_mask, or_mask);
//
//       // Since model_mask usually is not complete
...
//       float iou = static_cast<float>(overlap_size) / static_cast<float>(or_size);
//
//       const float model_ratio = static_cast<float>(overlap_size) / static_cast<float>(model_size);
//       const float mask_ratio = static_cast<float>(overlap_size) / static_cast<float>(mask_size);
//       float ratio = iou;
//       // ratio = fmaxf(model_ratio, mask_ratio);
//       // TODO: use this part using outside_chair_4 (Do not need to generate new obj when sudden appearance)
//       if (frame <= 5){
//         ratio /= 0.45;   // implicitly set the threshold to 0.45
//       }
//
//       // cv::imshow("model_instance_mask", model_instance_mask);
//       // cv::imshow("mask_instance_mask", mask_instance_mask);
//       // cv::imshow("overlap_mask", overlap_mask);
//       std::cout << "model ratio: " << model_ratio << ", mask ratio: " << mask_ratio << std::endl;
//       std::cout << "iou: " << iou << std::endl;
//
//
//       // Inclusion relation
//       if (mask_ratio > 0.95 && model_ratio < 0.7){
//         // inclusion_map.insert(std::make_pair(model_instance_id, mask_instance_mask));
//         std::cout << "inclusion check right" << std::endl;
//         inclusion_set.insert(model_instance_id);
//       }
//
//
...
//         is_match = true;
//         instance_seg& merged_update = mergedSeg.pair_instance_seg_.at(model_instance_id);
...
//           // implicitly change rendered_ to be false
...
//       }
//       // else
//       //    same class, yet the ratio is too low =>mainly for debuging
...
//     }
//
//     // If the src mask matches one of the model, and the inclusion set is not empty, set inclusion map
//     // if (is_match && !inclusion_set.empty()){
//     //   std::cout << "src mask matches and inclusion exists..." << std::endl;
//     //   for (auto it : inclusion_set){
//     //     inclusion_map.insert(std::make_pair(it, mask_instance_mask));
//     //   }
//     // }
//
//     // If this src mask is too small to associate with any other model masks,
//     // or the src mask did not exist in the model
...
bool Segmentation::local2global(SegmentationResult& mergedSeg,
                                ...
                                std::map<int, cv::Mat> &inclusion_map,
                                uint frame,
                                std::set<int> &matched_model_index){
  bool hasnewmodel = false;
  ...
  for (auto mask_instance = mask.pair_instance_seg_.begin();
    ...
    // std::cout << "Now come to instance id " << mask_instance_id
    //           << ", class id " << mask_class_id << "!!!!!!" << std::endl;
    ...
    const int mask_size = cv::countNonZero(mask_instance_mask);
    float max_ratio = 0;      // used for save the maximum ratio
    int matched_instance_id;  // model instance id which matches the local mask
    int matched_class_id;     // model instance class id which matches the local mask
    cv::Mat matched_instance_mask;      // model mask matched
    cv::Mat matched_overlap_mask;       // overlap mask matched (used for debug)
    std::set<int> inclusion_set;
    ...
    for (auto model_instance = model.pair_instance_seg_.begin();
      ...
      // Consider the truncated mask-rcnn results, using the rendered mask
      // if (ratio > 0.99f){
      //   // const float inv_ratio = fmaxf(1./model_ratio, 1./mask_ratio);
      //   const float inv_ratio = 1./mask_ratio;
      ...
      if (mask_ratio > 0.95 && model_ratio < 0.7){
        // inclusion_map.insert(std::make_pair(model_instance_id, mask_instance_mask));
        // std::cout << "inclusion relation right" << std::endl;
        inclusion_set.insert(model_instance_id);
      }
      ...
      if (ratio > max_ratio) {
        max_ratio = ratio;
        matched_instance_id = model_instance_id;
        matched_class_id = model_class_id;
        matched_overlap_mask = overlap_mask;
        matched_instance_mask = model_instance_mask;
      }
    }

    if (max_ratio > 0){
      // cv::imshow("model_instance_mask", matched_instance_mask);
      // cv::imshow("mask_instance_mask", mask_instance_mask);
      // cv::imshow("overlap_mask", matched_overlap_mask);
      // std::cout << "Max IoU: " << max_ratio << std::endl;
      // cv::waitKey(0);
    }

    // If IoU is greater than a threshold, the local mask is associated with this model mask
    if (max_ratio > combine_threshold){
      // cv::imshow("Matched local mask", mask_instance_mask);
      // cv::imshow("Matched global mask", matched_instance_mask);
      // if (frame > 270) cv::waitKey(0);
      instance_seg& merged_update = mergedSeg.pair_instance_seg_.at(matched_instance_id);
      if (merged_update.rendered_ == true){
        // do not merge the same class mask, directly insert
        std::cout<< "Mask with instance id " << matched_instance_id
                 << ", class id " << matched_class_id
                 << " coincident with the current object labels." << std::endl;

        cv::Mat globalMask = merged_update.instance_mask_.clone();
        // cv::Mat globalMask = matched_instance_mask.clone();
      ...
      merged.insert(matched_instance_id);
      mask2model_merged++;

      // Add tag for map object to indicate a match
      matched_model_index.insert(matched_instance_id);
    }

    // If the IoU of this src mask is too small to associate with any other model masks,
    // generate a new segmentation mask
    if (max_ratio < new_model_threshold){
      if (mask_size < min_mask_size) {
        std::cout << "Mask with instance id " << mask_instance_id
                  << ", class id " << mask_class_id
                  << " is too small to"
        ...
        // Add tag for map object to indicate a match
        matched_model_index.insert(new_generate_id);
      }
    }
  }
...
void Segmentation::remove_human_and_small(SegmentationResult& output,
  ...
  for(auto input_instance = input.pair_instance_seg_.begin();
    ...
    // TODO: Check whether need to add bitwise_or function and continue
    ...
    // std::cout << "mask name: " << input_instance->second.class_id_ << std::endl;
    // std::cout << "mask_size: " << mask_size << std::endl;
    // std::cout << "min_mask_size: " << min_mask_size << std::endl;
    if (mask_size < min_mask_size) {
      // cv::imshow("min_mask", input_mask);
      // cv::waitKey(0);
...
// Inclusion check
void SegmentationResult::inclusion_check() {
  SegmentationResult temp_copy = *this;
  for (auto & it_dst : this->pair_instance_seg_){
    int dst_instance_id = it_dst.first;
    if (dst_instance_id == 0) continue;
    if (dst_instance_id == INVALID) continue;
    cv::Mat &dst = it_dst.second.instance_mask_;

    for (auto & it_src : temp_copy.pair_instance_seg_){
      int src_instance_id = it_src.first;
      if (src_instance_id == 0) continue;
      if (src_instance_id == INVALID) continue;
      cv::Mat &src = it_src.second.instance_mask_;
      cv::Mat intersection_mask;
      cv::bitwise_and(src, dst, intersection_mask);
      int intersection_size = cv::countNonZero(intersection_mask);
      int src_size = cv::countNonZero(src);
      int dst_size = cv::countNonZero(dst);
      float src_ratio = static_cast<float>(intersection_size) / static_cast<float>(src_size);
      float dst_ratio = static_cast<float>(intersection_size) / static_cast<float>(dst_size);
      if (src_ratio > 0.95 && dst_ratio < 0.7){
        std::cout << "Find inclusion between instance id " << src_instance_id
                  << " and instance id " << dst_instance_id << std::endl;
        // Delete the inclusion part
        // cv::imshow("src", src);
        // cv::imshow("dst", dst);
        cv::bitwise_xor(dst, src, dst);
        // cv::imshow("final", this->pair_instance_seg_.at(dst_instance_id).instance_mask_);
        // cv::waitKey(0);
      }
    }
  }
