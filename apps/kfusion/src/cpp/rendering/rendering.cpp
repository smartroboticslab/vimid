...
void raycastDebug(ObjectList& objectlist, float3* vertex, float3* normal,
...
#pragma simd
    for (unsigned int x = 0; x < inputSize.x; x++) {
      ...
      for (auto object = objectlist.begin(); object != objectlist.end(); ++object){
        ...
        const float3 w_origin = get_translation(view);
        const float3 o_origin = inverse(T_w_o) * w_origin;
        ...
        const float4 o_hit = t_min > 0.f ?
                             raycast(volume, o_origin, o_direction,
                                     ...
                                     nearest_farPlane, mu, step, largestep, bg)
                                         : make_float4(0.f);
        ...
        const double fg_prob = volume.interp(make_float3(o_hit), [](const auto &val){ return val.fg; });

        if (fg_prob <= 0.5f) {
          // std::cout << "Fg prob for object " << label << " class id " << (*object)->class_id_
          //           << " is " << fg_prob << std::endl;
          miss_count++;
          if (label == 1){
            // debug.at<cv::Vec3b>(y,x)[2] = 255;
            if (fg_prob > 0) {
              // std::cout << "Current pose: " << x << " " << y << std::endl;
              // std::cout << "Current prob: " << fg_prob << std::endl;
              count++;
            }
          }
          continue;
          ...
          if (class_id == 57) debug.at<cv::Vec3b>(y,x)[2] = 255;
          if (class_id == 0) debug.at<cv::Vec3b>(y,x)[1] = 255;
        }
...
void raycastObjectList(ObjectList& objectlist, float3* vertex, float3* normal,
  ...
  TICK();
  unsigned int y;
  bool should_raycast_all = (objects_in_view.size() == 0 || objects_in_view.size() == 1);
  const Matrix4 view = T_w_c * getInverseCameraMatrix(k);
...
//        }
  ...
  // cv::Mat debug;
  // labelImg.convertTo(debug, CV_8U, 255.0);
  ...
  for (y = 0; y < depthSize.y; y++) {
    for (unsigned int x = 0; x < depthSize.x; x++) {
      ...
      if(doraycast) {
        ...
        if (miss_count == objectlist.size()){
         // std::cerr<< "RAYCAST MISS in all objects at position:"<< x << " " << y <<"\n";
  ...
  for (y = 0; y < framesize.y; y++) {
    ...
    for (unsigned int x = 0; x < framesize.x; x++) {
      ...
      if (use_icp && use_rgb){
        ...
        //              icp_error.result == -6 || rgb_error.result == -6;
        // is_inlier = ((icp_error.result>-3) && (rgb_error.result>-3)) ||
        //         (icp_error.result == -5 && rgb_error.result > -3) ||
        //         (rgb_error.result == -5 && icp_error.result > -3) ||
        //         (rgb_error.result == -5 && icp_error.result == -5);
        is_inlier = icp_error.result > -4 && rgb_error.result > -4 ||
                    icp_error.result == -6 || rgb_error.result == -6;
      }
      else{
        if ((use_icp) && (!use_rgb)){
...
bool check_truncated(const cv::Mat &object_mask, const uint2 framesize){
  unsigned int y;
  unsigned int x_min = framesize.x-1;
  unsigned int x_max = 0;
  unsigned int y_min = framesize.y-1;
  unsigned int y_max = 0;
// #pragma omp parallel for shared(object_mask), private(y)
  for (y = 0; y < framesize.y; y++) {
    for (unsigned int x = 0; x < framesize.x; x++) {
      if (object_mask.at<uchar>(y, x) != 0) {
        if (y < y_min) y_min = y;
        if (y > y_max) y_max = y;
        if (x < x_min) x_min = x;
        if (x > x_max) x_max = x;
  ...
  unsigned int x_avg = (x_min + x_max) / 2;
  unsigned int y_avg = (y_min + y_max) / 2;
  bool check_0 = (x_avg <= 30) || (x_avg >= framesize.x-31);
  bool check_1 = (y_avg <= 30) || (y_avg >= framesize.y-31);
  if (check_0 || check_1) return true;

  // Boundary threshold
  // bool check_0 = (x_min >= 0) && (x_min <= 5);  // 10
  // bool check_1 = (y_min >= 0) && (y_min <= 5);
  // bool check_2 = (x_max >= framesize.x-6) && (x_max <= framesize.x-1);
  // bool check_3 = (y_max >= framesize.y-6) && (y_max <= framesize.y-1);
  // std::cout << x_min << " "
  //           << x_max << " "
  //           << y_min << " "
  //           << y_max << std::endl;
  // if (check_0 || check_1 || check_2 || check_3) {
  //   return true;
  // }
...
float check_outlier_ratio(const cv::Mat& object_mask,
                          const TrackData* camera_tracking_result,
                          const uint2 framesize,
                          const bool use_icp,
                          const bool use_rgb){
  int icp_outlier_num = 0;
  int rgb_outlier_num = 0;
  unsigned int y;
  for (y = 0; y < framesize.y; y++) {
    int rowOffeset = y * framesize.x;
    for (unsigned int x = 0; x < framesize.x; x++) {
      ...
      bool icp_outlier = false;
      bool rgb_outlier = false;
      if (use_icp && use_rgb){
        icp_outlier = icp_error.result == -4 || icp_error.result == -5;
        rgb_outlier = rgb_error.result == -4 || rgb_error.result == -5;
      }
      else{
        if ((use_icp) && (!use_rgb)){
          icp_outlier = icp_error.result == -4 || icp_error.result == -5;
  ...
  float outlier_ratio = float(icp_outlier_num) / float(rgb_outlier_num);
  std::cout << "icp outlier num: " << icp_outlier_num << std::endl;
  std::cout << "rgb outlier num: " << rgb_outlier_num << std::endl;
  std::cout << "outlier ratio: " << outlier_ratio << std::endl;
  return outlier_ratio;
}

void check_static_state_with_ratio(cv::Mat& object_inlier_mask,
                                   const cv::Mat& object_mask,
                                   const TrackData* camera_tracking_result,
                                   const uint2 framesize,
                                   const bool use_icp,
                                   const bool use_rgb,
                                   float &valid_icp_ratio){
  int icp_outlier_num = 0;
  int icp_inlier_num = 0;
  unsigned int y;
  for (y = 0; y < framesize.y; y++) {
    int rowOffeset = y * framesize.x;
    for (unsigned int x = 0; x < framesize.x; x++) {
      ...
      bool is_inlier = false;
      bool is_icp_outlier = false;
      if (use_icp && use_rgb){
        is_inlier = icp_error.result > -4 && rgb_error.result > -4 ||
                    icp_error.result == -6 || rgb_error.result == -6;
        is_icp_outlier = icp_error.result <= -3;
      }
      else{
        if ((use_icp) && (!use_rgb)){
          is_inlier = (icp_error.result>-3);
  ...
  valid_icp_ratio = float(icp_inlier_num) / float(icp_inlier_num + icp_outlier_num);
}