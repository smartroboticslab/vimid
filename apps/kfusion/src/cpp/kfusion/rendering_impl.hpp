  ...
  if (tnear < tfar) {
    ...
    if (f_t > 0) { // ups, if we were already in it, then don't render anything here
      for (; t < tfar; t += stepsize) {
        ...
        if (f_tt < 0) {                 // got it, jump out of inner loop
          // Eliminate the noise hit
          // float t_new = t + stepsize * f_tt / (f_t - f_tt);
          float t_new = t;
          float4 result = make_float4(origin + direction * t_new, t_new);
          const double fg_prob = volume.interp(make_float3(result), [](const auto &val){ return val.fg; });
          const double weight = volume.interp(make_float3(result), [](const auto &val){ return val.w; });
          if ((fg_prob < 0.5 || weight < 1) && !bg) {
            stepsize = largestep;
            position += stepsize*direction;
            continue;
          }
