 ...
 * SPDX-FileCopyrightText: 2017-2023 Smart Robotics Lab, Imperial College London
...
void bilateralFilterKernel(float *out, const float *in, uint2 size,
						   const float *gaussian, float e_d, int r, float depth_max)
{
	TICK()
	uint y;
...
#pragma omp parallel for shared(out), private(y)
	for (y = 0; y < size.y; y++)
	{
		for (uint x = 0; x < size.x; x++)
		{
			...
			// 3.0 5.0
			if (depth_max > 0 && in[pos] > depth_max)
			{
				// Cut off input dist > 3.0
				out[pos] = 0;
				continue;
			}
...
cv::Mat resizeMat(const uchar3 *input, uint2 inputSize, uint2 computationSize)
{
	int ratio = inputSize.x / computationSize.x;
	cv::Mat result = cv::Mat::zeros(cv::Size(computationSize.x, computationSize.y), CV_8UC3);
	unsigned int y;
	for (y = 0; y < computationSize.y; y++)
	{
		for (unsigned int x = 0; x < computationSize.x; x++)
		{
			...
			result.at<cv::Vec3b>(y, x)[2] = input[pos].x; // R
			result.at<cv::Vec3b>(y, x)[1] = input[pos].y; // G
			result.at<cv::Vec3b>(y, x)[0] = input[pos].z; // B
		}
	}
	return result;
}
