...
void drawit(T* scene, uint2 size) {
    ...
    // glutInitWindowSize(320 * 2, 240 * 2);
		...
		glutCreateWindow("Main View Display");
	}
...
void drawthem(A* scene1, B* scene2, C* scene3, D* scene4, E* scene5, F* scene6,
	...
	if (lastsize.x != size_s2.x || lastsize.y != size_s2.y) {
		...
		glutCreateWindow("VI-MID Display");
		glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
...
void drawNineScene(A* scene1, B* scene2, C* scene3, D* scene4, E* scene5,
                   F* scene6, G* scene7, H* scene8, I* scene9,
                   uint2 size_s1, uint2 size_s2, uint2 size_s3, uint2 size_s4, uint2 size_s5,
                   uint2 size_s6, uint2 size_s7, uint2 size_s8, uint2 size_s9) {
  static uint2 lastsize = { 0, 0 };
  char * t = (char*) "toto";
  int g = 1;
  if (lastsize.x != size_s3.x || lastsize.y != size_s3.y) {
    lastsize = size_s3;
    glutInit(&g, &t);
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB);
    ...
    gluOrtho2D(0.0, (GLfloat) 960, 0.0, (GLfloat) 720);
    glMatrixMode(GL_MODELVIEW);
  ...
  glRasterPos2i(0, 720);
  glPixelZoom(320.0 / size_s7.x, -240.0 / size_s7.y);
  glDrawPixels(size_s7.x, size_s7.y, gl<G>::format, gl<G>::type, scene7);

  glRasterPos2i(320, 720);
  glPixelZoom(320.0 / size_s8.x, -240.0 / size_s8.y);
  glDrawPixels(size_s8.x, size_s8.y, gl<H>::format, gl<H>::type, scene8);

  glRasterPos2i(640, 720);
  glPixelZoom(320.0 / size_s9.x, -240.0 / size_s9.y);
  glDrawPixels(size_s9.x, size_s9.y, gl<I>::format, gl<I>::type, scene9);
  glutSwapBuffers();
}