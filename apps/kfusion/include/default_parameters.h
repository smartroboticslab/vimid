...
const std::string default_okvis_config_file = "";
const std::string default_okvis_dataset_path = "";
const bool default_okvis_mapKeyFrameOnly = false;
const bool default_use_imu = false;
const float default_depth_max = 5.0f;
...
//     "bc:d:ef:g:hi:l:m:k:o:p:r:s:t:v:y:z:B:FC:M:S:R:D:P:O:G:T:A:H:V:I";

static std::string short_options = "a:bc:d:f:g:hi:k:l:m:o:p:qr:s:t:uv:y:z:AB:CDFG:HI:J:MO:PR:S:V:W:X:Y:";
...
static struct option long_options[] =
{
  ...
  {"geom-refine-human",       no_argument,       0, 'H'},
  {"init",                    required_argument, 0, 'I'},
  {"depth-max",               required_argument, 0, 'J'},
  ...
  {"okvis-config-file",       required_argument, 0, 'W'},
  {"okvis-dataset-path",      required_argument, 0, 'X'},
  {"okvis-mapKeyFrameOnly",   required_argument, 0, 'Y'},
  {0, 0, 0, 0}
};
...
void print_arguments() {
  ...
  std::cerr << "-u  (--use-imu)                  : default is " << default_use_imu                                << std::endl;
  ...
  std::cerr << "-R  (--ground-truth-segment) <foldername>   : Groundtruth segmentation folder"                    << std::endl;
  ...
  std::cerr << "-W  (--okvis-config-file)        : default is " << default_okvis_config_file                      << std::endl;
  std::cerr << "-X  (--okvis-dataset-path)       : default is " << default_okvis_dataset_path                     << std::endl;
  std::cerr << "-Y  (--okvis-mapKeyFrameOnly)    : default is false"                                              << std::endl;
  std::cerr << "-J  (--depth-max)                : default is 5.0f (meter). If set nonpositive, no truncation"    << std::endl;
}
...
Configuration parseArgs(unsigned int argc, char ** argv) {
  ...
  config.okvis_config_file = default_okvis_config_file;
  config.okvis_dataset_path = default_okvis_dataset_path;
  config.okvis_mapKeyFrameOnly = default_okvis_mapKeyFrameOnly;
  config.use_imu = default_use_imu;
  ...
  config.depth_max = default_depth_max;
  ...
  while ((c = getopt_long(argc, argv, short_options.c_str(), long_options, &option_index)) != -1)
    switch (c) {
      ...
      case 'c':     //  -c  (--compute-size-ratio)
        ...
        std::cerr << "update compute_size_ratio to " << config.compute_size_ratio << std::endl;
      ...
      case 'g':     //  -g  (--ground-truth)
        ...
        std::cerr << "using the groundtruth file: " << config.groundtruth_file << std::endl;
        break;
      case 'h':     //  -h  (--bayesian)
      ...
      case 'l':     //  -l  (--icp-threshold)
        ...
        std::cerr << "update icp_threshold to " << config.icp_threshold << std::endl;
        break;
      case 'm':     //  -m  (--mu)
      ...
      case 'o':     //  -o  (--log-file)
        ...
        std::cerr << "update log_file to " << config.log_file << std::endl;
        break;
      case 'p':     //  -p  (--init-pose)
      ...
      case 'r':     //  -r  (--integration-rate)
        ...
        std::cerr << "update integration_rate to " << config.integration_rate << std::endl;
      ...
      case 't':     //  -t  (--tracking-rate)
        ...
        std::cerr << "update tracking_rate to " << config.tracking_rate << std::endl;
        break;
      case 'u':
        config.use_imu = true;
        std::cerr << "activate use-imu" << std::endl;
        break;
      ...
      case 'I':     //  -I  (--init)
        ...
        break;
      case 'J':     //  -J  (--depth-max) to truncate input depth images
        config.depth_max = atof(optarg);
        std::cerr << "Truncating the input depth images within " << config.depth_max << " meters" << std::endl;
        break;
      case 'M':     //  -M  (--multi-res)
        config.multiResolution = true;
        std::cerr << "using multi-resolution integration" << std::endl;
        break;
      case 'O':     //  -O  (--output-images)
      ...
      case 'V':     //  -V  (--obj-moved)
        ...
        break;
      case 'W':     //  -W  (--okvis-config-file)
        config.okvis_config_file = optarg;
        std::cerr << "update okvis config file to " << config.okvis_config_file << std::endl;
        break;
      case 'X':     //  -X  (--okvis-dataset-path)
        config.okvis_dataset_path = optarg;
        std::cerr << "update okvis dataset path to " << config.okvis_dataset_path << std::endl;
        break;
      case 'Y':     //  -Y  (--okvis-mapKeyFrameOnly)
        config.okvis_mapKeyFrameOnly = true;
        break;
      case 0:
      case '?':
