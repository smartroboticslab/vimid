...
inline Eigen::Vector4f homogeneous(float3 v){
  return (Eigen::Vector4f(v.x, v.y, v.z, 1));
}

inline void save_mesh_ply(const std::vector<Triangle>& mesh,
  ...
  file << "end_header\n";
  // const Eigen::Vector3i rgb = (255.0f * color_fraction_map[0]).cast<int>();
  // Write vertices and vertex data
  for (size_t i = 0; i < num_faces; ++i ) {
    const Triangle& triangle_M = mesh[i];
    // Write each triangle vertex
    for (int v = 0; v < 3; ++v) {
      const Eigen::Vector3f vertex_W = (T_WM * homogeneous(triangle_M.vertexes[v])).head(3);
  ...
  for (size_t i = 0; i < num_faces; ++i ) {
    ...
    // const Triangle& triangle_M = mesh[i];
    const Eigen::Vector3i rgb = (255.0f * color_fraction_map[1]).cast<int>();
    file << " " << rgb.x() << " " << rgb.y() << " " << rgb.z();
    if (has_cell_data) {
      file << " " << cell_data[i] << "\n";
    } else {
